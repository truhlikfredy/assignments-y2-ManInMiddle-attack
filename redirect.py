...
def request(context, flow):
  ...
  if hostPretty == "www.wit.ie":
    ...
    domainTrick = True
    domainTrick = (False for x in request.headers if x[0] == "User-Agent" and ("Chrome" in x[1] or "Safari" in x[1]))
    
    # fix for lazy students who can't remember lecturers name after a year
    data = response.read().replace("RLACEY@wit.ie", "RFRISBY@wit.ie")
